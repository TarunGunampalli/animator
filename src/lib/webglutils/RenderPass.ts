import { WebGLUtilities } from "./CanvasAnimation.ts";
...
class Uniform {
	...
	constructor(location: WebGLUniformLocation, bindFunction: (gl: WebGLRenderingContext, loc: WebGLUniformLocation) => void) {
...
class Attribute {
	...
	constructor(name: string, size: number, type: GLenum, normalized: boolean, stride: number, offset: number, bufferName: string) {
		this.name = name;
		this.size = size;
		this.type = type;
